...
# Save to ROOT (Parent of backend folder)
OUTPUT_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'database.json')
...
def main():
    ...
    try:
        ...
        for line in lines:
            ...
            if line.startswith("#EXTINF:"):
                ...
                if url.startswith('http'):
                    # Prefer HTTPS if available (simple replace for knowns, or just accept)
                    # For GitHub Pages (HTTPS), HTTP streams often fail (Mixed Content).
                    # We will prioritize valid links.
                    current['url'] = url
        ...
        print(f"💾 Saved to ROOT: {OUTPUT_FILE}")
